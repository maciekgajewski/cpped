...
template<typename SplitterDirection>
void splitter<SplitterDirection>::set_streatching(unsigned idx, event_window* win)
{
	entries_[idx].window_ = win;
...
{
	if (entries_[0].type == entry_type::unset && entries_[1].type == entry_type::unset)
	{
		return; // do nothing
	}
	...
	unsigned size0 = 0;
	unsigned size1 = 0;
	if(entries_[0].type == entry_type::unset)
	{
		size0 = 0;
		size1 = size-2;
	}
	else if (entries_[1].type == entry_type::unset)
	{
		size0 = size-2;
		size1 = 0;
	}
	...
	{
		if (entries_[0].type == entry_type::fixed && entries_[0].type == entry_type::stretching)
		{
			size0 = size_of_fixed(size, entries_[0].requested_size_) - 1;
			size1 = size - size0 - 2;
		}
		else if (entries_[0].type == entry_type::fixed && entries_[0].type == entry_type::stretching)
		{
	...
	if(entries_[0].window)
	{
		SplitterDirection::apply_size(entries_[0].window, 1, size0);
	}
	if(entries_[1].window)
	{
		SplitterDirection::apply_size(entries_[1].window, size0 + 2, size1);
	}
...
template<>
class splitter<_splitter_directions::horizontal>;

template<>
class splitter<_splitter_directions::vertical>;
